                    ...
                    if (transaction.getId() == transactionIndex) {
                        ...
                        if (transaction.getRecipient() == null) {
                            if (account.getBalance() >= transaction.getAmount()) {
                                double balancee = account.getBalance() - transaction.getAmount();
                                account.setBalance(balancee);
                                account.setBalance(account.getBalance() - transaction.getAmount());
                                Transaction revert = new Transaction(account, -transaction.getAmount(), postedAt,
                            ...
                            Transaction revert = new Transaction(account, recipient, -transaction.getAmount(),
                                    postedAt, "REVERT: " + transaction.getNote());
                            account.addTransaction(revert);
                        }
                        break;
